...
int main()
{
    ...
    do
    {
        std::cout << "\nMenue:" << std::endl
				  ...
				  << "(A): Sortieren der Studenten nach Matr.Nr." <<std::endl
                  << "(0): Beenden" << std::endl;
        std::cin >> abfrage;
        std::cin.ignore(12, '\n');
        ...
        switch (abfrage)
        {
            ...
            case '5':
            {
				if(!studentenListe.empty())
				{
					...
					std::vector<Student>::iterator s;

					s = std::find(studentenListe.begin(), studentenListe.end(), matNr);
