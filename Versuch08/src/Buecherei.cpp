...
void ausgeliheneMedienAusgeben(std::vector<Medium*>& medien);
...
int main()
{
    ...
    do  // Anzeige des Men�s
    {
        std::cout << std::endl
				  ...
				  << "(6): Ausgelihene Medien ausgeben" << std::endl
                  << "(7): Beenden" << std::endl;
        ...
        switch (abfrage)
        {
            ...
            ////////////////////////////////////////
            // Ausgelihene Medien ausgeben //
            ////////////////////////////////////////
            case '6':
            {
                ausgeliheneMedienAusgeben(medien);
                break;
            }
...
////////////////////////////////////////////////////
// Funktion gibt alle ausgelihenen Medien in der Datenbank aus //
////////////////////////////////////////////////////
void ausgeliheneMedienAusgeben(std::vector<Medium*>& medien)
{
	std::vector<Medium*> geliheneMedien;

	for (unsigned int index = 0; index < medien.size(); index++)
	    {
			if(medien[index]->getStatus()) geliheneMedien.push_back(medien[index]);
	    }
	if(geliheneMedien.size()==0){
		std::cout << "Keine Medien ausgelihen!" << std::endl;
		return;
	}
    std::cout << "Ausgelihne Medien in der B�cherei:" << std::endl;

    for (unsigned int index = 0; index < geliheneMedien.size(); index++)
    {
		std::cout << "*************************************************************" << std::endl;
		medien[index]->ausgabe();
    }
